...
const apiUrl = `https://localhost:7023/api/Senzors/check/`;
const apiUrlNotify = `https://localhost:7023/api/SignalRealTime/notify`;
const apiUrlCheck = `https://localhost:7023/api/SignalRealTime`;
 
client.subscribe("sendingHttpCalls", async function( {task, taskService})  {
    ...
    fetch(url, {
 ...
 
      {
        ...
        console.log("sensorWorking " + isWorking);
 
 
        const processVariables = new Variables()
 
            .set("sensorWorking", isWorking);
...
client.subscribe("SendingEmails", async function( {task, taskService})  {
 
    //httpPOSTNotify + sensorid + alert
 
    let url = `${apiUrlNotify}`;
